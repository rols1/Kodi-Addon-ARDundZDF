...
class Stream(object):
    ...
    # 05.01.2020 ergänzt mit only_url (kein Download, nur Rückgabe der Url)
    def download(self, output_path=None, filename=None, filename_prefix=None, only_url=None):
    ...
    def on_progress(self, chunk, file_handler, bytes_remaining):
        ...
        if on_progress:
            PLog('streams: calling on_progress callback %s', on_progress)
    ...
    def on_complete(self, file_handle):
        ...
        """
        PLog('streams: download finished')
        on_complete = self._monostate['on_complete']
        if on_complete:
            PLog('streams: calling on_complete callback %s', on_complete)
